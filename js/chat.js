...
let allChatsCache = [];
let groupMembers = [];
let isChatOpen = false; // Flag status obrolan aktif
...
// ============================================================================
// BUKA & TUTUP OBROLAN (FIX SINKRONISASI KIWI BROWSER)
// ============================================================================
...
async function openChat(chatId, preloadedData) {
  ...
  // Tambahkan state riwayat browser HANYA jika chat baru dibuka
  if (!isChatOpen) {
    isChatOpen = true;
...
// FUNGSI UNTUK MENUTUP CHAT SECARA BERSIH
function closeChat(fromPopState = false) {
  if (!isChatOpen && !currentChatId) return;
  ...
  // Jika ditutup dari tombol '←' UI, bersihkan riwayat browser secara manual
  if (!fromPopState && history.state && history.state.inChat) {
    history.back();
  }
}
...
// ============================================================================
// PREVIEW FOTO + NAVIGASI MOBILE (EVENT LISTENERS)
// ============================================================================
...
// KLIK TOMBOL "←" DARI APLIKASI
const backBtn = document.getElementById("backBtn");
if (backBtn) {
  backBtn.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    closeChat(false); // false = ditutup via tombol UI
  });
}

// BILA MENGGUNAKAN TOMBOL BACK HP / SWIPE BACK
window.addEventListener("popstate", () => {
  if (isChatOpen) {
    closeChat(true); // true = ditutup via navigasi HP
  }
});
